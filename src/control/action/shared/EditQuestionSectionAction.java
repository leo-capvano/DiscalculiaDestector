 ...
 * @version 1.0
 */
public class EditQuestionSectionAction extends Action {
    ...
    public EditQuestionSectionAction() {
    ...
    protected String processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException, ParseException {
        ...
        String numOfChoicesString = request.getParameter("numChoices");
        ...
        if(description != null)
            questionSection.setDescription(description);
        if(numOfChoicesString != null) {
            int numOfChoices = Integer.parseInt(numOfChoicesString);
            int maxNumChoices = questionSection.getQuestionType().getMaxMultipleChoices();
            
            if(numOfChoices > maxNumChoices) {
                jsonResponse = new JsonResponse(false, "Il numero di scelte deve essere al massimo " + maxNumChoices);
                writer.println(gson.toJson(jsonResponse));
                return null;
            }
            else
                questionSection.setNumOfChoices(numOfChoices);
        }
        
        questionSectionModel.doUpdate(questionSection);
    ...
    protected boolean validateRequest(HttpServletRequest request, HttpServletResponse response) {
        ...
        String numOfChoices = request.getParameter("numChoices");
    ...
    protected void catchInvalidRequestError(HttpServletResponse response) throws IOException {
        ...
        jsonResponse.setJsonResponseMessage("Invalid request");
        writer.println(gson.toJson(writer));
    }
