...
public class DeleteQuestionAction extends Action {
    ...
    protected String processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException, ParseException {
        ...
        Question question = questionModel.doRetrieveById(questionID);
        if(question == null) {
        ...
        if(loggedAccount == null || loggedAccount instanceof DyscalculiaPatient || (!(loggedAccount instanceof Administrator) && question.getQuestionSection().getQuiz().isTrusted()) ) {
            jsonResponse = new JsonResponse(false, UNAUTHORIZED_MESSAGE);
            writer.println(gson.toJson(jsonResponse));
            return null;
        }
         
        questionModel.doDelete(question);
